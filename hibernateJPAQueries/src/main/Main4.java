...
public class Main4 {
	...
	public static void main(String[] args) {
		...
		CriteriaQuery<CdWrapper> criteria = builder.createQuery(CdWrapper.class);
		...
		criteria.select(builder.construct(CdWrapper.class, titulo, anyo));
		...
		List<CdWrapper> cds = em.createQuery(criteria).getResultList();

		s.getTransaction().commit();

		for (CdWrapper cd : cds) {
